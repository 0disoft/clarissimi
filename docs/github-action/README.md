...
`v0.6.2`. That release executes the committed `action-dist/index.js` bundle without consumer-time
...
full-write consumer smoke for immutable tag `v0.6.2` and moving alias `v0`.
...
`v0.6.2` includes both table and gallery layouts.
...
`promote-draft` is available in immutable tag `v0.6.2`, which passed external consumer smoke. Do not
