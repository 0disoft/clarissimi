...
`0disoft/clarissimi@v0.5.0` when exact patch reproducibility matters. The moving `main` ref is
...
plus write modes `commit` and `stage-draft`. The current `v0.5.0` release executes the committed
...
full-write consumer smoke for immutable tag `v0.5.0` and moving alias `v0`.
...
`v0.5.0` includes both table and gallery layouts.
...
`promote-draft` is available in immutable tag `v0.5.0`, which passed external consumer smoke. Do not
