...
`0disoft/clarissimi@v0.6.1` when exact release reproducibility matters. The moving `main` ref is
...
`dry-run` plus write modes `commit` and `stage-draft`. Gate mode is available in immutable release
`v0.6.1`. That release executes the committed `action-dist/index.js` bundle without consumer-time
package installation or TypeScript compilation. `v0.1.0` remains
...
full-write consumer smoke for immutable tag `v0.6.1` and moving alias `v0`.
...
`clarissimi.config.ts` executes as runner code. Load it only from a trusted checkout, never from an
untrusted pull request head or a path derived from pull request data. A `pull_request_target`
workflow that uses TypeScript config must keep the trusted base revision checked out.
...
`v0.6.1` includes both table and gallery layouts.
...
Gate mode is available from immutable release `v0.6.1`:
...
`promote-draft` is available in immutable tag `v0.6.1`, which passed external consumer smoke. Do not
