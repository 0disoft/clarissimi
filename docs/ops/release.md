...
table coverage, internal workspace dependency graph, TypeScript project-reference build graph, the
...
trigger, permission, preflight, runtime, and command drift, `git diff --check`, and a high-risk
secret pattern scan. It does not call live providers and does not replace the credentialed smoke
gates below.
