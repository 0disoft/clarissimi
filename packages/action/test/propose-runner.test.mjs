...
test("propose mode preserves existing ledger records when rendering a new contribution", async () => {
  await withTempDir(async (dir) => {
    ...
    const records = (await readFile(join(stagingDir, ".clarissimi", "contributions.jsonl"), "utf8"))
      .trim()
      .split("\n")
...
test("environment runner writes bounded propose outputs and step summary", async () => {
  await withTempDir(async (dir) => {
    ...
    assert.equal(
      outputText.includes("proposal-pull-request-url=https://github.com/sample/project/pull/1"),
      true,
    );
...
test("environment runner writes bounded stage-draft outputs and step summary", async () => {
  await withTempDir(async (dir) => {
    ...
    assert.equal(
      outputText.includes("proposal-branch=clarissimi/drafts/merged_pull_request-42"),
      true,
    );
...
async function commitExistingLedger(repositoryDir, record) {
  ...
  await writeFile(join(ledgerDir, "contributions.jsonl"), `${JSON.stringify(record)}\n`, "utf8");
