...
export async function runBundleAction(options = {}) {
  ...
  if (checkOnly) {
    ...
    if (trackedText !== bundledText) {
      throw new Error("Tracked Action bundle is stale. Run `pnpm run bundle:action` and commit action-dist/index.js.");
    }
...
if (process.argv[1] !== undefined && pathToFileURL(process.argv[1]).href === import.meta.url) {
  await runBundleAction({ checkOnly: process.argv.slice(2).includes("--check") });
}
