...
async function run(argv, runtime) {
  ...
  const evidenceArgs = [
    "run", "release-candidate-evidence-issue", "--",
    "--release-type", releaseType,
    "--sha", sha,
    "--ci-run", String(ciRun.databaseId),
    "--live-run", String(liveRun.databaseId),
    "--external-run", String(externalRun.databaseId),
  ...
  await command(runtime, "pnpm", evidenceArgs, args.createIssue ? "create evidence issue" : "render evidence issue preview", { inherit: true });
...
function parseArgs(argv, runtime) {
  ...
  const valueOptions = new Map([
    ["repo", "repo"], ["branch", "branch"], ["external-repo", "externalRepo"], ["external-ref", "externalRef"],
    ["release-type", "releaseType"], ["release-version", "releaseVersion"], ["sha", "sha"],
    ["provider-model", "providerModel"], ["provider-endpoint", "providerEndpoint"], ["provider-thinking", "providerThinking"]
  ]);
  for (let index = 0; index < argv.length; index += 1) {
    const arg = argv[index];
    if (arg === "--") continue;
    if (arg === "--help" || arg === "-h") { parsed.help = true; continue; }
    if (!arg.startsWith("--")) return usageFailure(runtime, `Unexpected positional argument: ${arg}`);
    const key = arg.slice(2);
    if (booleanOptions.has(key)) { parsed[booleanOptions.get(key)] = true; continue; }
...
function validateArgs(args, runtime) {
  if (args.providerModel === undefined || args.providerModel.trim() === "") return usageFailure(runtime, "--provider-model is required.");
  const releaseType = args.releaseType ?? defaults.releaseType;
  if (!["source-only", "versioned-action-tag"].includes(releaseType)) return usageFailure(runtime, "--release-type must be source-only or versioned-action-tag.");
  if (releaseType === "versioned-action-tag" && !/^v\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(args.releaseVersion ?? "")) {
    return usageFailure(runtime, "versioned-action-tag evidence requires --release-version <v0.x.y>.");
  }
  if (args.providerEndpoint !== undefined && !isHttps(args.providerEndpoint)) return usageFailure(runtime, "--provider-endpoint must be an https URL.");
  if (args.providerThinking !== undefined && args.providerThinking !== "disabled") return usageFailure(runtime, "--provider-thinking supports only disabled.");
}

async function requireSecret(runtime, repo, name) {
  const output = await commandText(runtime, "gh", ["secret", "list", "--repo", repo, "--app", "actions", "--json", "name"], "list repository secret names");
  const secrets = parseJson(output, "gh secret list");
  if (!Array.isArray(secrets) || !secrets.some((secret) => secret.name === name)) throw new Error(`Missing repository secret ${name} for ${repo}. No workflow was dispatched.`);
...
async function watchIfNeeded(runtime, repo, run, label) {
  ...
  if (run.status === "completed") throw new Error(`${label} failed: conclusion=${run.conclusion ?? "unknown"} (${run.url}).`);
  await command(runtime, "gh", ["run", "watch", String(run.databaseId), "--repo", repo, "--exit-status"], `watch ${label}`, { inherit: true });
}
...
async function command(runtime, executable, args, label, options = {}) {
  ...
  if (result.exitCode !== 0) throw new Error(`Unable to ${label}: exit code ${result.exitCode}.${options.inherit ? "" : `\n${bounded(result.stderr)}`}`);
}

function appendOption(args, flag, value) { if (value !== undefined) args.push(flag, value); }
...
function runCommand(commandName, args, options = {}) {
  return new Promise((resolve, reject) => {
    const child = spawn(commandName, args, { stdio: options.inherit ? "inherit" : ["ignore", "pipe", "pipe"] });
