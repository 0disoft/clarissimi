...
function validateFullWriteRun(run, runId) {
  ...
  if (run.event !== "workflow_dispatch")
    throw new Error(`Full-write run ${runId} must use workflow_dispatch.`);
  if (run.status !== "completed")
    throw new Error(
      `Full-write run ${runId} must be completed before cleanup; status=${run.status ?? "unknown"}.`,
    );
  if (
    typeof run.displayTitle !== "string" ||
    !run.displayTitle.startsWith(`${defaults.workflowName} · `) ||
    !run.displayTitle.endsWith(` · ${runId}`)
  ) {
  ...
  if (typeof run.url !== "string" || !run.url.startsWith("https://github.com/"))
    throw new Error(`Full-write run ${runId} is missing a GitHub Actions URL.`);
}
...
async function readCleanupState(runtime, repo, expectedBranches) {
  ...
  if (pullRequestsResult.exitCode !== 0)
    throw new Error(
      `Unable to list open pull requests for ${repo}.\n${bounded(pullRequestsResult.stderr)}`,
    );
  ...
  return {
    pullRequests: pullRequests
      .filter(
        (pullRequest) =>
          expectedBranches.has(pullRequest.headRefName) ||
          expectedBranches.has(pullRequest.baseRefName),
      )
...
class UsageError extends Error {
  constructor() {
    super("Invalid release evidence cleanup arguments.");
    this.exitCode = 2;
  }
}
