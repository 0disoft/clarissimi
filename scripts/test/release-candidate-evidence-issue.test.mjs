...
test("release candidate evidence issue resolves HEAD and creates an issue with body on stdin", async () => {
  ...
  const issueCreate = harness.commands.find((command) =>
...
test("release candidate evidence issue rejects workflow run mismatch before issue creation", async () => {
  ...
  assert.equal(
    wrongSha.errors.includes("hosted CI run 12345 validates ffffffffffffffffffffffffffffffffffffffff, not 0123456789abcdef0123456789abcdef01234567."),
    true
  );
  assert.equal(
    ...
    runtime: {
      ...
      runCommand: async (command, args, commandOptions = {}) => {
        ...
        if (matches(args, ["issue", "create"])) {
          return success(`${options.issueUrl ?? "https://github.com/0disoft/clarissimi/issues/1"}\n`);
        }
