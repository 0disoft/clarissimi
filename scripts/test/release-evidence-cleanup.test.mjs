...
test("previews only residue deterministically owned by the completed full-write run", async () => {
  ...
  assert.deepEqual(receipt.matched.pullRequests.map((item) => item.number), [41]);
  assert.deepEqual(receipt.matched.branches, [draft, recognition, base].sort());
  assert.equal(harness.commands.some((item) => isMutation(item.args)), false);
});
...
test("apply closes matched pull requests before deleting exact branches and verifies empty state", async () => {
  ...
  assert.deepEqual(mutations.map((item) => item.args.slice(0, 3)), [
    ["pr", "close", "41"],
    ["api", "--method", "DELETE"],
    ["api", "--method", "DELETE"]
  ]);
...
test("rejects unrelated or active workflow runs before reading repository state", async () => {
  for (const run of [
    ...
    assert.equal(harness.commands.some((item) => item.args[0] === "pr"), false);
  }
});
...
function createHarness(options = {}) {
  const commands = []; const logs = []; const errors = [];
  let stateReads = 0;
  return {
    ...
    runtime: {
      ...
      runCommand: async (command, args) => {
        ...
        if (args[0] === "api" && args[1]?.startsWith("repos/")) {
          const state = stateReads === 0 ? options : (options.stateAfterMutation ?? options);
          stateReads += 1;
          return success(JSON.stringify((state.branches ?? []).map((branch) => ({ ref: `refs/heads/${branch}` }))));
        }
...
function pr(number, headRefName, baseRefName) {
  return { number, headRefName, baseRefName, url: `https://github.com/0disoft/integration-lab/pull/${number}` };
}
function isMutation(args) { return (args[0] === "pr" && args[1] === "close") || (args[0] === "api" && args[1] === "--method"); }
function success(stdout) { return { exitCode: 0, stdout, stderr: "" }; }
function failure(stderr) { return { exitCode: 1, stdout: "", stderr }; }
