...
import {
  ...
  stableActionReleaseToolingContract,
...
test("release readiness rejects release policy document drift", () => {
  const text = createReleasePolicyText()
    ...
    .replace("release, Marketplace, validation-record,", "release tooling is absent")
    .replace(
    ...
    .replace("## Stable v1 Release Procedure", "## Ungated v1 Release")
    .replace(
  ...
  assert.deepEqual(validateReleasePolicyDocumentContract(text), [
    ...
    "docs/ops/release.md must include release, Marketplace, validation-record,.",
    ...
    "docs/ops/release.md must include - Stable root Action tag: `v1.0.0` is selected by ADR 0055; v1-capable release tooling is.",
    ...
    "docs/ops/release.md must include pnpm run promote-action-major-alias -- --release-version <v0.x.y|v1.x.y> --sha <commit-sha>.",
    ...
    "docs/ops/release.md must include pnpm run publish-action-release -- --version <v0.x.y|v1.x.y> --sha <candidate-sha>.",
...
test("release readiness accepts stable v0 and v1 release tooling synchronization", () => {
  assert.deepEqual(
    validateStableActionReleaseToolingContract(createStableActionReleaseToolingTexts()),
    [],
  );
});

test("release readiness rejects stable Action release tooling drift", () => {
  const texts = createStableActionReleaseToolingTexts();
  const first = stableActionReleaseToolingContract.documents[0];
  delete texts[first.path];
  const second = stableActionReleaseToolingContract.documents[1];
  texts[second.path] = texts[second.path].replace(second.requiredSnippets[0], "");

  assert.deepEqual(validateStableActionReleaseToolingContract(texts), [
    `${first.path} must be readable for stable Action release tooling contract.`,
    `${second.path} must include ${second.requiredSnippets[0]}.`,
  ]);
});
...
test("release readiness rejects README validation drift", () => {
  const text = createReadmeValidationText()
    ...
    .replace("- `pnpm run verify-marketplace-release -- --version <v0.x.y|v1.x.y>`", "")
    .replace(
      "- `pnpm run promote-action-major-alias -- --release-version <v0.x.y|v1.x.y> --sha <commit-sha>`",
      "",
    )
  ...
  assert.deepEqual(validateReadmeValidationContract(text), [
    ...
    "README.md must include - `pnpm run hosted-external-consumer-smoke -- --clarissimi-ref <v0|v1> --expected-sha <commit-sha>`.",
    "README.md must include - `pnpm run verify-action-major-tag -- --release-version <v0.x.y|v1.x.y> --sha <commit-sha>`.",
    "README.md must include - `pnpm run verify-marketplace-release -- --version <v0.x.y|v1.x.y>`.",
    "README.md must include - `pnpm run promote-action-major-alias -- --release-version <v0.x.y|v1.x.y> --sha <commit-sha>`.",
...
test("release readiness rejects ledger format document drift", () => {
  const text = createLedgerFormatDocumentText()
    ...
    .replace(
      ...
      "Public ledger records may keep draft metadata.",
    )
    .replace(
      "They must not store AI agent, delegated model, prompt,",
      "They may store model names, prompts,",
    )
    .replace(
      "CLI draft commands sanitize public records so provenance does not",
      "CLI draft commands preserve provenance in public records",
    );
...
test("release readiness rejects CLI configuration document drift", () => {
  const text = createCliConfigurationDocumentText()
    ...
    .replace(
      ...
      "The CLI always reads provider tokens.",
    );
...
test("release readiness rejects agent-assisted drafts document drift", () => {
  const text = createAgentAssistedDraftsDocumentText()
    ...
    .replace(
      ...
      "The public ledger stores model data.",
    );
...
test("release readiness rejects incident response document drift", () => {
  ...
  assert.deepEqual(validateIncidentResponseDocumentContract(text), [
    ...
    "docs/ops/incident-response.md must include Token, private key, raw provider output, raw diff, or sensitive evidence is public..",
...
test("release readiness rejects disaster recovery document drift", () => {
  const text = createDisasterRecoveryDocumentText()
    .replace(
      ...
      "Clarissimi disaster recovery covers hosted outages,",
    )
    .replace(
...
test("release readiness rejects Action inputs and outputs document drift", () => {
  const text = createActionInputsOutputsDocumentText()
    ...
    .replace(
      "reads `GITHUB_TOKEN` in `gate`, `propose`, `commit`,",
  ...
  assert.deepEqual(validateActionInputsOutputsDocumentContract(text), [
    "docs/github-action/inputs-and-outputs.md must include - `mode`: `gate`, `dry-run`, `propose`, `commit`, `stage-draft`, or `promote-draft`, default `propose`.",
...
test("release readiness rejects Action contract document drift", () => {
  const text = createActionContractDocumentText()
    ...
    .replace("ADR 0055 defines the stable `v1` line.", "No stable v1 contract exists.")
    .replace("require a non-draft and non-prerelease GitHub Release", "allow draft prereleases")
    .replace(
      "independent from the persisted `clarissimi.assessment/v1` schema identifier",
      "coupled to the persisted schema version",
    )
    .replace(
  ...
  assert.deepEqual(validateActionContractDocumentContract(text), [
    ...
    "docs/github-action/action-contract.md must include ADR 0055 defines the stable `v1` line..",
...
test("release readiness protects the deterministic scale benchmark document contract", () => {
  ...
  assert.deepEqual(
    validatePerformanceBenchmarkDocumentContract(
      text.replace("atomically committing all four repository outputs", "writing output files"),
    ),
    [
      "docs/engineering/03-performance-budget.md must include atomically committing all four repository outputs.",
    ],
  );
});
...
test("release readiness rejects engineering validation document drift", () => {
  ...
  texts["docs/engineering/03-performance-budget.md"] = texts[
  ...
  texts["docs/engineering/04-security-baseline.md"] = texts[
    ...
    validateWorkspacePackageTsconfigReferences(
      {
        ...
        references: [{ path: "../core" }, { path: "../renderers" }],
      },
      "providers",
      "packages/providers/tsconfig.json",
    ),
...
test("release readiness rejects package ownership ADR reference drift", () => {
  const text = createPackageOwnershipText().replace(
...
test("release readiness rejects Action manifest env and command drift", () => {
  const text = createActionManifestText()
    .replace(
      ...
      "CLARISSIMI_PROVIDER_TOKEN: ${{ inputs.provider-token }}",
    )
    .replace(
...
test("release readiness scopes Action manifest input and output checks to their sections", () => {
  const missingInputMode = createActionManifestText().replace(
    "  mode:\n    required: false\n    default: propose\n",
    "",
  );
  ...
  const missingOutputMode = createActionManifestText().replace(
...
test("release readiness rejects Action manifest output value drift", () => {
  const text = createActionManifestText().replace(
...
function createReleasePolicyText() {
  return [
    ...
    "https://github.com/0disoft/clarissimi/issues/21",
...
function createStableActionReleaseToolingTexts() {
  return Object.fromEntries(
    stableActionReleaseToolingContract.documents.map((document) => [
      document.path,
      document.requiredSnippets.join("\n"),
    ]),
  );
...
function createLedgerFormatDocumentText() {
  return [
    ...
    "",
    "Public ledger records are assessment-only.",
    "`contributor.kind` may be `human`, `bot`, or `ai_agent`; it is optional.",
    "Display opt-out never removes these records from the ledger.",
    "They must not store AI agent, delegated model, prompt,",
...
function createActionContractDocumentText() {
  return [
    ...
    "Release tools require a non-draft and non-prerelease GitHub Release and keep the Action major",
    "independent from the persisted `clarissimi.assessment/v1` schema identifier.",
    ...
    "`gate` is an opt-in pre-merge mode governed by ADR 0057.",
    "Human-readable text may follow the marker; the JSON marker remains the validated source of truth.",
    "",
    ...
    "Required mode fails for missing, stale, malformed, untrusted, duplicate, or",
    "",
    "Dry-run mode should need read permissions only.",
...
function createActionManifestText() {
  return [
    ...
    "  direct-commit-base-sha:",
    "    value: ${{ steps.clarissimi.outputs.direct-commit-base-sha }}",
    "  direct-commit-sha:",
    ...
    "  gate-mode:",
    "    value: ${{ steps.clarissimi.outputs.gate-mode }}",
    "  gate-passed:",
    "    value: ${{ steps.clarissimi.outputs.gate-passed }}",
    "  gate-decision:",
    "    value: ${{ steps.clarissimi.outputs.gate-decision }}",
    "  gate-reason:",
    "    value: ${{ steps.clarissimi.outputs.gate-reason }}",
    "runs:",
    "  using: composite",
    "  steps:",
    "    - name: Run Clarissimi",
