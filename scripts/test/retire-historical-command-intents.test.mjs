...
test("retirement removes completed mutations and keeps exact-tag revalidation manual-only", () => {
  const input = [
    'schema_version = "1"',
    sharedIntents,
    '[intents.release_v0_3_5_stable_publish]\nstatus = "configured"\nrun_policy = "agent_allowed"',
    '[intents.promote_v0_to_v0_3_5]\nstatus = "configured"\nrun_policy = "agent_allowed"',
    '[intents.git_stage_provider_model_eval]\nstatus = "configured"\nrun_policy = "agent_allowed"',
    '[intents.hosted_ci_provider_model_eval]\nstatus = "configured"\nrun_policy = "agent_allowed"',
    '[intents.release_v0_3_5_post_tag_evidence]\nstatus = "configured"\nlifecycle = "oneshot"\nrun_policy = "agent_allowed"\ndescription = "Revalidate."\nrequired_after = ["release_v0_3_5_stable_publish"]',
  ].join("\n\n");

  const result = retireHistoricalCommandIntents(input);

  assert.deepEqual(result.removed, [
    "release_v0_3_5_stable_publish",
    "promote_v0_to_v0_3_5",
    "git_stage_provider_model_eval",
    "hosted_ci_provider_model_eval",
  ]);
  assert.deepEqual(result.manualOnly, ["release_v0_3_5_post_tag_evidence"]);
  assert.ok(!result.text.includes("release_v0_3_5_stable_publish"));
