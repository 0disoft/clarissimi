...
test("validateDocs rejects stale public Action release documentation", async (t) => {
  ...
  await writeFile(readmePath, readme.replaceAll("v0.5.2", "v0.4.0"), "utf8");
  ...
  assert.equal(
    result.issues.includes(
      "README.md is missing current-state contract text: 0disoft/clarissimi@v0.5.2",
    ),
    true,
  );
});
...
test("validateDocs rejects stale gallery and supported-release lines", async (t) => {
  ...
  await writeFile(
    readmePath,
    readme.replace(
      "`gallery` is available in the current immutable `v0.5.2` release and moving `v0` line.",
  ...
  assert.equal(
    result.issues.includes(
      "README.md is missing current-state contract text: `gallery` is available in the current immutable `v0.5.2` release and moving `v0` line.",
    ),
    true,
  );
  assert.equal(
    result.issues.includes("SECURITY.md is missing current-state contract text: `v0.5.2`"),
    true,
  );
  assert.equal(
    result.issues.includes(
...
function stubContentForPath(path) {
  ...
  if (path === "SECURITY.md") {
    return "# Security\n\nSupported immutable release `v0.5.2` and moving `v0` release line.\n";
  }
...
function withReadmeContracts(readme) {
  return [
    readme.trimEnd(),
    "",
    "Current release: 0disoft/clarissimi@v0.5.2",
    "`gallery` is available in the current immutable `v0.5.2` release and moving `v0` line.",
