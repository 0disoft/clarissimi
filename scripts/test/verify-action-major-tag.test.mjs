...
test("verify action major tag accepts matching alias, immutable tag, and release", async () => {
  ...
  assert.deepEqual(harness.commands.map(({ command, args }) => [command, ...args.slice(0, 2)]), [
    ["git", "--version"],
    ["gh", "--version"],
    ["git", "ls-remote", "--tags"],
    ["gh", "release", "view"]
...
test("verify action major tag rejects a mismatched moving alias", async () => {
  const harness = createHarness({ aliasSha: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" });
  const exitCode = await runVerifyActionMajorTag([
  ...
  assert.equal(harness.commands.some(({ command }) => command === "gh"), true);
  assert.equal(harness.commands.some(({ args }) => args[0] === "release"), false);
});
